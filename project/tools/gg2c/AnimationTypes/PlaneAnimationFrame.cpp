...
PlaneAnimationFrame::PlaneAnimationFrame()
: m_FrameDelayTime(-1),
  m_FrameNumber(-1),
  m_uniqueTileCount(0)
{
...
void PlaneAnimationFrame::Init(int frameNumber, 
								 ...
								 int& maxUniqueTileCountPerFrame)
{
	...
	PaletteUtils::GetPaletteData(palette, m_palette);
}

void PlaneAnimationFrame::GetFrameDelayTime(const GraphicsGaleObject& ggo)
{
...
void PlaneAnimationFrame::BuildFrame(const GraphicsGaleObject& ggo, std::vector<Tile>& tiles, const Options& options, int& uniqueTileCount, int& maxUniqueTileCountPerFrame)
{
	HBITMAP				hBitmap;
	BITMAP				bitmapInfo;
...
bool PlaneAnimationFrame::hasSameFrameData(const PlaneAnimationFrame* frame)
{
