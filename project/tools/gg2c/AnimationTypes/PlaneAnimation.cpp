...
PlaneAnimation::PlaneAnimation(const GraphicsGaleObject& ggo, const Options& options)
: m_ggo(ggo),
  m_uniqueTileCount(0),
  ...
  m_numberOfFrames(ggo.getFrameCount())
{
    ...
    {
		PlaneAnimationFrame& frame = m_frames[loop];

		frame.Init(loop, ggo, m_tiles, m_options, m_uniqueTileCount, m_maxUniqueTilesPerFrame);
...
PlaneAnimationFrame* PlaneAnimation::findSameFrame(const PlaneAnimationFrame& frame)
{
...
int PlaneAnimation::findSameFrameIndex(const PlaneAnimationFrame& frame)
{
...
void PlaneAnimation::Write(const std::string& outputFolder, const std::string& outputName)
{
	WritePlaneAnimationHeaderFile(outputFolder, outputName);
	WritePlaneAnimationSourceFile(outputFolder, outputName);
}

void PlaneAnimation::WritePlaneAnimationHeaderFile(const std::string& outputFolder, const std::string& outputName)
{
	...
	if (m_options.mFixedBack)
	{
		headerfile << "#define " << outputName << "LoadFixed() VDPTileManager_LoadPlaneAnimationToVDPFixed(&" << outputName << ", " << outputName << "VDPTileIndex)\n\n";
	}
	else
	{
		headerfile << "#define " << outputName << "Load(region) VDPTileManager_LoadPlaneAnimationToVDP(region, &" << outputName << ", &" << outputName << "VDPTileIndex)\n\n";
	}
	*/
...
void PlaneAnimation::WriteFrameData(const std::string& outputName, std::ofstream& sourceFile)
{
	for (size_t loop = 0; loop < m_uniqueFrameData.size(); loop++)
	{
		std::string frameName = BuildFrameDataName(outputName, loop);

		const PlaneAnimationFrame* frame = m_uniqueFrameData[loop];
		//
...
void PlaneAnimation::WritePalettes(const std::string& outputName, std::ofstream& sourceFile)
{
	for (size_t frameLoop = 0; frameLoop < m_uniqueFrameData.size(); frameLoop++)
	{
		...
		const PlaneAnimationFrame* frame = m_uniqueFrameData[frameLoop];
...
void PlaneAnimation::WriteTileMaps(const std::string& outputName, std::ofstream& sourceFile)
{
	for (size_t frameLoop = 0; frameLoop < m_uniqueFrameData.size(); frameLoop++)
	{
...
void PlaneAnimation::WriteFrameNames(const std::string& outputName, std::ofstream& sourceFile)
{
...
void PlaneAnimation::WriteFrames(const std::string& outputName, std::ofstream& sourceFile)
{
	for (size_t loop = 0; loop < m_frames.size(); loop++)
	{
...
void PlaneAnimation::WriteFrameArray(const std::string& outputName, std::ofstream& sourceFile)
{
...
void PlaneAnimation::WriteAnimationStruct(const std::string& outputName, std::ofstream& sourceFile)
{
    // final struct
    sourceFile << "\n";
    sourceFile << "\n";
...
void PlaneAnimation::WritePlaneAnimationSourceFile(const std::string& outputFolder, const std::string& outputName)
{
	std::ofstream sourceFile(outputFolder + outputName + ".c");

    // includes
	sourceFile << "// File generated by gg2c. \n";
    sourceFile << "#include \"" << outputName << ".h\"\n";
    sourceFile << "\n";
    sourceFile << "\n";

	WriteTileData(outputName, sourceFile, m_tiles.size(), m_tiles);
