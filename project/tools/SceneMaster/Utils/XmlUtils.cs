...
namespace SceneMaster.Utils
{
    internal class XmlUtils
    {
        ...
        public static T GetChildValue<T>(XmlElement node, string childNodeName, string attributeName = "value", T defaultValue  = default(T))
        {
            ...
            if (!string.IsNullOrEmpty(attributeValue) && TryParse<T>(attributeValue, out var parsedValue))
            {
                return parsedValue;
            }
            else
